...
class TestRefreshPodigeeToken:
    ...
    def test_successful_refresh(self):
        ...
        with patch(
            "manager.podigee_connector.requests.post", return_value=mock_response
        ) as mock_post:
    ...
    def test_refresh_token_already_used(self):
        ...
        mock_response.raise_for_status.side_effect = requests.HTTPError(
            "400 Client Error: invalid_grant"
        )
    ...
    def test_successful_refresh_then_reuse_failure(self):
        ...
        mock_failure_response.raise_for_status.side_effect = requests.HTTPError(
            "400 Client Error: invalid_grant"
        )
        ...
        with patch(
            "manager.podigee_connector.requests.post",
            side_effect=[mock_success_response, mock_failure_response],
        ) as mock_post:
...
class TestHandlePodigeeRefresh:
    ...
    def test_successful_refresh_and_db_update(self, mock_encrypt, mock_refresh):
        ...
        assert result is not None
        assert (
            result["PODIGEE_ACCESS_TOKEN"]
            == "new_access_token_with_sufficient_length_for_validation"
        )
        assert (
            result["PODIGEE_REFRESH_TOKEN"]
            == "new_refresh_token_with_sufficient_length_for_validation"
        )
    ...
    def test_same_refresh_token_warning(self, mock_refresh):
        ...
        with patch("manager.podigee_connector.encrypt_json"):
            ...
            assert (
                result["PODIGEE_REFRESH_TOKEN"]
                == "old_refresh_token_with_sufficient_length_for_validation"
            )
    ...
    def test_token_comparison_logic(self, mock_encrypt, mock_refresh):
        ...
        with patch("manager.podigee_connector.logger") as mock_logger:
            ...
            mock_logger.info.assert_any_call(
                "Successfully obtained new refresh token for Test Podcast"
            )
...
class TestTokenReuseIssues:
    ...
    @patch("manager.podigee_connector.refresh_podigee_token")
    @patch("manager.podigee_connector.encrypt_json")
    def test_database_transaction_failure_leaves_consumed_token(
        self, mock_encrypt, mock_refresh
    ):
        """
        ...
        mock_refresh.assert_called_once_with(
            "client_id", "client_secret", "valid_but_will_be_consumed_token", None
        )
    ...
    @patch("manager.podigee_connector.refresh_podigee_token")
    @patch("manager.podigee_connector.encrypt_json")
    def test_simulated_second_program_run_with_consumed_token(
        self, mock_encrypt, mock_refresh
    ):
        """
        ...
        assert (
            call_args_list[0][0][2] == "original_token_that_will_be_consumed"
        )  # First call
        assert (
            call_args_list[1][0][2] == "original_token_that_will_be_consumed"
