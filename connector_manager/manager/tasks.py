...
def process_spotify_podcast(account_id, source_podcast_id, source_access_keys, pod_name):
    ...
    """
    logger.info(f"Processing Spotify task for {pod_name} {account_id} using podcast_id {source_podcast_id}")
    ...
    try:
        ...
        os.environ["SPOTIFY_PODCAST_ID"] = source_access_keys.get("SPOTIFY_PODCAST_ID", source_podcast_id)
        
        # Default environment setup
        BASE_URL = source_access_keys.get("SPOTIFY_BASE_URL", "https://generic.wg.spotify.com/podcasters/v0")
        SPOTIFY_CLIENT_ID = source_access_keys.get("SPOTIFY_CLIENT_ID", "05a1371ee5194c27860b3ff3ff3979d2")
        SP_DC = source_access_keys.get("SPOTIFY_SP_DC")
        SP_KEY = source_access_keys.get("SPOTIFY_SP_KEY")
        ...
        # Date range setup
        START_DATE = source_access_keys.get("START_DATE", (dt.datetime.now() - dt.timedelta(days=4)).strftime("%Y-%m-%d"))
        END_DATE = source_access_keys.get("END_DATE", (dt.datetime.now() - dt.timedelta(days=1)).strftime("%Y-%m-%d"))
        date_range = get_date_range(START_DATE, END_DATE)
        
        # Check required variables
        missing_vars = []
        if not SP_DC: missing_vars.append("SPOTIFY_SP_DC")
        if not SP_KEY: missing_vars.append("SPOTIFY_SP_KEY") 
        if not source_podcast_id: missing_vars.append("PODCAST_ID")
        ...
        # Process all endpoints with worker threads
        queue = Queue()
        ...
        logger.info(f"Successfully processed Spotify podcast {pod_name} {account_id}")
        return {
            "status": "success",
        ...
        logger.error(f"Spotify credentials expired for {pod_name} {account_id}: {e}")
        raise
    except Exception as e:
        logger.error(f"Exception while processing Spotify podcast {pod_name}: {e}")
        raise
    finally:
        # Clean up sys.path
...
def process_podigee_podcast(account_id, source_podcast_id, source_access_keys, pod_name):
    ...
    """
    logger.info(f"Processing Podigee task for {pod_name} {account_id} using podcast_id {source_podcast_id}")
    ...
    try:
        ...
        OPENPODCAST_API_ENDPOINT = os.environ.get("OPENPODCAST_API_ENDPOINT", "https://api.openpodcast.dev")
        OPENPODCAST_API_TOKEN = source_access_keys.get("OPENPODCAST_API_TOKEN")
        BASE_URL = source_access_keys.get("PODIGEE_BASE_URL", "https://app.podigee.com/api/v1")
        PODIGEE_ACCESS_TOKEN = source_access_keys.get("PODIGEE_ACCESS_TOKEN")
        PODIGEE_USERNAME = source_access_keys.get("PODIGEE_USERNAME")  
        PODIGEE_PASSWORD = source_access_keys.get("PODIGEE_PASSWORD")
        ...
        try:
            PODCAST_ID = int(source_podcast_id)
        except ValueError:
            raise ValueError(f"PODCAST_ID must be an integer, got: {source_podcast_id}")
        ...
        logger.info(f"Successfully processed Podigee podcast {pod_name} {account_id}")
        return {
            "status": "success",
            "account_id": account_id,
            "source_name": "podigee",
        ...
        logger.error(f"Exception while processing Podigee podcast {pod_name}: {e}")
        raise
    finally:
        # Clean up sys.path
...
def process_apple_podcast(account_id, source_podcast_id, source_access_keys, pod_name):
    ...
    """
    logger.info(f"Processing Apple task for {pod_name} {account_id} using podcast_id {source_podcast_id}")
...
def process_anchor_podcast(account_id, source_podcast_id, source_access_keys, pod_name):
    ...
    """
    logger.info(f"Processing Anchor task for {pod_name} {account_id} using podcast_id {source_podcast_id}")
...
def process_podcast_task(account_id, source_name, source_podcast_id, source_access_keys_encrypted, pod_name):
    ...
    # Dispatch to appropriate connector task
    if source_name == "spotify":
        return process_spotify_podcast(account_id, source_podcast_id, source_access_keys, pod_name)
    elif source_name == "podigee": 
        return process_podigee_podcast(account_id, source_podcast_id, source_access_keys, pod_name)
    elif source_name == "apple":
        return process_apple_podcast(account_id, source_podcast_id, source_access_keys, pod_name)
    elif source_name == "anchor":
        return process_anchor_podcast(account_id, source_podcast_id, source_access_keys, pod_name)
    else:
