...
def refresh_podigee_token(client_id, client_secret, refresh_token, redirect_uri=None):
    ...
    try:
        ...
        logger.info(
            f"Successfully refreshed Podigee access token, expires in {token_data.get('expires_in')} seconds"
        )
...
def handle_podigee_refresh(
    ...
    # Refresh the token
    token_data = refresh_podigee_token(
    ...
    if (
        ...
        logger.error(
            "This likely means the refresh token was already used or is invalid."
        )
        logger.error(f"Original refresh token was: {original_refresh_token[:10]}...")
        logger.error(
            "Manual intervention may be required to re-authenticate the Podigee connection."
        )
        return None
    ...
    if new_refresh_token == original_refresh_token:
        logger.warning(
            f"Podigee returned the same refresh token for {pod_name}. This is unusual."
        )
    else:
    ...
    try:
        with db_connection.cursor() as cursor:
            ...
            if cursor.rowcount == 0:
                logger.error(
                    f"No rows were updated - account_id '{account_id}' or source_name 'podigee' not found"
                )
                logger.error(
                    f"New refresh token that could not be saved: {token_data['refresh_token']}"
                )
                return None
        ...
        logger.error(
            f"New refresh token that could not be saved: {token_data['refresh_token']} Account ID: {account_id}, Pod name: {pod_name}"
        )
        logger.error(
            "Manual intervention required: update the refresh token in the database manually."
        )
        return None
